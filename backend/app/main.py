...
from .imports import ImportValidationError, ROLE_HEADERS, TEMPLATE_ROWS, parse_uploads, preview_payload, validate_and_build
from .models import (
    DashboardResponse,
    ImportCommitRequest,
    QueryRequest,
...
async def import_preview(request: Request, files: list[UploadFile] = File(...)) -> dict:
    try:
...
def dashboard_endpoint(request: Request) -> DashboardResponse:
    return dashboard(_context(request))
...
async def query_endpoint(payload: QueryRequest, request: Request) -> QueryResponse:
    ...
    return await run_copilot(context, payload.question, workflow)
