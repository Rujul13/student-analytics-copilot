...
FEATURE_NAMES = ("prior_average", "prior_success_rate", "prior_withdrawals", "prior_credits", "course_level")
...
class SuccessModel:
    ...
    def predict(self, features: list[float]) -> float:
        vector = np.asarray([1.0, *features], dtype=float)
...
def _feature_rows(context: DatasetContext) -> pd.DataFrame:
    ...
    levels = context.frames["courses"][["course_code", "level"]].drop_duplicates("course_code")
    joined = enrollments.merge(grades, on="enrollment_id", how="left").merge(levels, on="course_code", how="left")
    ...
    for student_id, history in joined.groupby("student_id"):
        ...
        for current in history.itertuples():
            ...
            level = min(max(float(current.level) if pd.notna(current.level) else 1.0, 1.0), 3.0) / 3
            rows.append({
...
def get_success_model(context: DatasetContext) -> SuccessModel | None:
    ...
    x_test = np.column_stack([np.ones(len(test)), test[list(FEATURE_NAMES)].to_numpy(dtype=float)])
    y_test = test["success"].to_numpy(dtype=float)
    probabilities = 1 / (1 + np.exp(-np.clip(x_test @ weights, -20, 20)))
    evaluation = SuccessEvaluation(
        model_name="Temporal academic-history logistic baseline",
    ...
    model = SuccessModel(weights=weights, evaluation=evaluation)
    _MODEL_CACHE[context.version] = model
    return model


def candidate_features(context: DatasetContext, student_id: str, average_grade: float, withdrawals: int, credits_earned: int, level: int) -> list[float]:
    ...
    return [
        average_grade / 100 if len(history) else 0.5,
        success_rate,
        min(withdrawals, 3) / 3,
