...
def main() -> None:
    ...
    for learner in learner_summaries:
        representatives.setdefault(learner.risk, learner)
    observed_codes = set(context.frames["enrollments"]["course_code"])
    ...
    for learner in representatives.values():
        ...
        for item in result.recommendations:
            assert item.course_code in observed_codes
            assert item.course_code not in completed | current
            assert 0 <= item.score <= 100
            assert item.historical_records > 0
    ...
    plans = [
        AnalyticsPlan(intent="metric", metric="completion_rate", risk_band=None, limit=None, sort_descending=None, student_id=None, minimum_failed_courses=None),
        AnalyticsPlan(intent="student_risk_table", metric=None, risk_band="High", limit=5, sort_descending=False, student_id=None, minimum_failed_courses=None),
        AnalyticsPlan(intent="module_performance", metric=None, risk_band=None, limit=3, sort_descending=True, student_id=None, minimum_failed_courses=None),
    ]
